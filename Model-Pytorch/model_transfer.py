...
from data_utils_transfer_revised import get_character_vector  # 토치 텐서를 반환하도록 수정
...
class Model(nn.Module):
    def __init__(self, hparams, data_loader):
        ...
        # 수정: 파라미터 이름 중 'corrector'가 들어간 것은 제외하고 필터링
        main_params = [p for name, p in self.named_parameters() if 'corrector' not in name]
        
        # Initialize the optimizer (필터링된 파라미터만 전달)
        self.optimizer = torch.optim.AdamW(
            main_params, 
            lr=self.learning_rate, 
            eps=hparams['epsilon'], 
            weight_decay=self.l2_reg_coef
        )
