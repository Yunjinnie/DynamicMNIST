...
from utils_joint import get_batch_iterations
from data_utils_joint import import_hparams, BatchGenerator, DataLoader
...
def train(hparams, logger, checkpoint):
    ...
    for epoch in range(epoch_first, n_epochs):
        ...
        for step, batch_train in tqdm(enumerate(generator(inputs_train, targets_train)), desc=f'[Train] Iterations (epoch {epoch})', initial=0, total=iterations_per_epoch):
            ...
            #pred_train, cr_pred_train, states_train, h_seqs, smoothing_ratios, loss_train, sub_losses_train, cr_loss_train = model.train_step(inputs, targets, mask, cr_inputs_train)
            #print('pred_train:', pred_train.shape) # pred_train: torch.Size([128, *, 104])
            # 외부에서는 Main Model의 입력만 넣어주면 알아서 내부에서 Corrector 데이터까지 다 만들어서 돌림!
            pred_train, cr_pred_train, states_train, h_seqs, smoothing_ratios, loss_train, sub_losses_train, cr_loss_train = model.train_step(inputs, targets, mask)
