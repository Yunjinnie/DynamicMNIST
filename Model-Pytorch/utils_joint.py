...
def revised_gmm_batch_pt(z_pi, z_mu1, z_mu2, z_sigma1, z_sigma2, z_corr, z_hov=None, z_eod=None, numpy_output=True, only_gmm=False):
    # (텐서 변환 과정은 원본과 동일하게 유지)
    if isinstance(z_pi, np.ndarray):
        # 경고 방지를 위해 데이터 타입을 강제로 float32 등으로 맞추는 것이 좋습니다.
        z_pi = torch.tensor(z_pi, dtype=torch.float32)
    # 들어오는 모든 NumPy 변수들을 PyTorch 텐서로 변환...
    ...
    # ---------------------------------------------------------
    # 1. 인덱스 샘플링 (Categorical -> Gumbel-Softmax 대체)
    # ---------------------------------------------------------
    # dist_categorical.sample()은 미분이 불가능하므로 Gumbel-Softmax의 Hard 모드를 사용합니다.
    # Forward 시에는 One-hot 벡터(예: [0, 1, 0])를 반환하여 하드 샘플링을 수행하고,
    # Backward 시에는 Softmax의 그래디언트를 통과시키는 마법 같은 기법입니다.
    
    # log_pi를 구함 (안전성을 위해 1e-8 추가)
    logits = torch.log(z_pi + 1e-8)
    
    # tau(온도)는 보통 1.0부터 시작하여 점진적으로 낮추기도 하지만 1.0도 무방합니다.
    # hard=True 옵션이 핵심입니다. (Forward는 이산적, Backward는 연속적)
    gumbel_weights = F.gumbel_softmax(logits, tau=1.0, hard=True) # [batch_size, n_mixtures]

    # ---------------------------------------------------------
    # 2. 파라미터 추출 (Gather -> 행렬 곱 대체)
    # ---------------------------------------------------------
    # One-hot 벡터(gumbel_weights)를 곱해서 더하면 인덱스 선택과 완벽히 동일한 결과가 나옵니다.
    # (미분 그래프가 gumbel_weights를 타고 z_pi로 흘러갑니다.)
    mu1 = torch.sum(gumbel_weights * z_mu1, dim=-1)       # [batch_size]
    mu2 = torch.sum(gumbel_weights * z_mu2, dim=-1)
    sigma1 = torch.sum(gumbel_weights * z_sigma1, dim=-1)
    sigma2 = torch.sum(gumbel_weights * z_sigma2, dim=-1)
    corr = torch.sum(gumbel_weights * z_corr, dim=-1)
    
    # ---------------------------------------------------------
    # 3. Reparameterization Trick (원본 유지 - 이미 미분 가능함)
    # ---------------------------------------------------------
    z1 = torch.randn_like(mu1)
    z2 = torch.randn_like(mu2)
    ...
    # ---------------------------------------------------------
    # 4. Hover, EOD 샘플링 (Bernoulli -> Straight-Through Estimator 대체)
    # ---------------------------------------------------------
    hov_prob = z_hov.squeeze(-1) if z_hov is not None else torch.zeros_like(dx)
    eod_prob = z_eod.squeeze(-1) if z_eod is not None else torch.zeros_like(dx)
    
    # 그냥 torch.bernoulli를 쓰면 미분이 끊깁니다. STE를 구현합니다.
    # Forward: 실제 베르누이 샘플링 결과 (0.0 or 1.0)
    # Backward: 확률값(prob)의 그래디언트를 그대로 복사해서 넘김
    hov_sample = torch.bernoulli(hov_prob)
    hov = (hov_sample - hov_prob).detach() + hov_prob 

    eod_sample = torch.bernoulli(eod_prob)
    eod = (eod_sample - eod_prob).detach() + eod_prob 

    if numpy_output:
        return dx.detach().cpu().numpy(), dy.detach().cpu().numpy(), hov.detach().cpu().numpy(), eod.detach().cpu().numpy()
    return dx, dy, hov, eod
...
def sample_gmm_seqs_pt(pred, hparams):
    ...
    var_x, var_y, hov, eod = revised_gmm_batch_pt(z_pi, z_mu1, z_mu2, z_sigma1, z_sigma2, z_corr, z_hov, z_eod, numpy_output=False)
