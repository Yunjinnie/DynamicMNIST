...
from dtw_classifier_transfer import DTWClassifier
...
class Logger:
    ...
    def init_dtw_classifier(self, val_set):
        ### error
        # 현재 logger가 가지고 있는 character_set (이미 stage에 따라 10개 혹은 52개로 맞춰둔 상태)을 전달
        ...
        # character_set=self.character_set # 이 부분을 명시적으로 전달하도록 수정되어 있는지 확인
    ...
    def compute_stat_between_human_sets(self, train_set, val_set, test_set):
        ...
        for character in self.character_set: # # 여기서 모든 문자를 다 돌고 있음
            for metric in metrics:
                ...
                # [핵심 수정] 데이터가 없으면 통계 계산 자체를 하지 않음
                if p_samples is None or len(p_samples) == 0:
                    # print(f"DEBUG: Skipping stats for {character} (No data in this stage)")
                    continue

                if q_samples is None or len(q_samples) == 0:
                    # print(f"DEBUG: Skipping stats for {character} (No data in this stage)")
                    continue
    ...
    def log_test_synth(self, itr, epoch_float, model, test_set):
        ...
        # [추가] 실제 테스트 셋에 포함된 문자들만 리스트업 (예: '0'~'9')
        active_characters = sorted(list(set(characters_test)))
        # 수정: self.character_set 대신 active_characters 사용

        stat_test_data_character, stat_test_data_all = get_stat_data_from_test_set(active_characters, test_set, self.dx_norm, self.dy_norm) # self.character_set
        # 1. 실제 데이터가 있는 문자들만 필터링 (0~9 등)
        # stat_test_data_character의 키값들만 모으면 데이터가 있는 문자만 남음
        existing_chars = sorted(list(stat_test_data_character.keys()))
        ...
        for character in active_characters: #self.character_set:
            # 모델 인퍼런스 호출 (PyTorch)
            ...
            for metric in metrics:
                ...
                if len(p_samples) > 0:
                    ...
                    # 펜으로 그린 데이터가 하나도 없을 경우 기본값(0 또는 NaN) 처리
                    print("[Warning] No valid samples generated in this batch. Skipping stats calculation.")
                    # (선택) 로깅 시스템이 p_stats의 값을 꼭 필요로 한다면 임의의 더미 값을 넣어주세요.
                    p_stats = None
                    q_stats = None
                #p_stats = stats.describe(p_samples)
                #q_stats = stats.describe(q_samples)
        ...
        pdf_paths, png_paths = plot_behavior_distributions(
            existing_chars, #self.character_set,
        ...
        for character in accuracy_character.keys():
            ##### error
            # 사람 데이터(기준점)에 해당 문자가 있을 때만 Gap 계산
            if character in self.accuracy_character:
                ...
                # Stage 1에서 알파벳이 분류된 경우 등, 기준값이 없으면 스킵하거나 0 처리
                print(f"ℹ [Skip Gap] Character '{character}' not in test set targets. Skipping gap calculation.")


        # 1. [순서 변경] 사람 데이터(Test Set Target) 먼저 계산!
        ...
        # 3. Gap 계산 (이제 self.precisions_cnn_test가 존재하므로 안전합니다)
        ...
        # CNN 관련 수정 (precisions_cnn_test 등이 기준점)
        # 이제 안전하게 체크 가능!
        for character in precisions.keys():
            # 기준점인 self.precisions_cnn_test에 해당 문자가 있는지 확인
            if hasattr(self, 'precisions_cnn_test') and character in self.precisions_cnn_test:
                wandb.log({
                ...
                # 기준점이 없으면 기본 precision/recall 등만 기록하고 gap은 생략
                wandb.log({
                    'test/cnn-classification/prediction-SF/precision-character-{}'.format(character): precisions[character],
