...
from data_utils_joint import get_character_vector  # 토치 텐서를 반환하도록 수정
from utils_joint import revised_gmm_batch_pt, sample_gmm_batch, sample_gmm_batch_pt, parse_pred
...
class Model(nn.Module):
    ...
    def train_step(self, x, y, mask):
        ...
        # Phase 2: On-the-fly Corrector Data Generation (그래디언트 보존)
        # (Corrector의 forward에서 detach가 제거되었으므로, 여기서 계산된 그래프는 Main Model과 연결됨)
        # 방금 나온 따끈따끈한 y_pred를 이용해 즉석에서 코렉터용 데이터셋을 생성
        # (내부적으로 Gumbel-Softmax를 쓰기 때문에 미분 그래프가 끊기지 않고 y_pred와 연결!)
        cr_inputs, cr_targets, cr_mask = self.dl.generate_dataset_for_corrector(y_pred, states, h_seqs, x, y, mask)
        
        # 혹시 디바이스가 틀어질 수 있으니 안전하게 맞춰줌
        cr_inputs = cr_inputs.to(self.device)
        cr_targets = cr_targets.to(self.device)
        cr_mask = cr_mask.to(self.device)

        # Phase 3: Corrector Forward & Joint Loss
        cr_pred, corrections, _, _ = self.corrector(cr_inputs)
        loss_corr = self.corrector.loss_fn(corrections, cr_targets, cr_mask)

        # Phase 4: Backpropagation
        # 3. Loss 합산 및 Lambda 스케일링 (Residual Correction 기법)
        lambda_corr = 0.1 # 0.1 ~ 0.5 사이에서 튜닝 (Corrector의 영향력 제어), Main Model 보호를 위한 가중치 스케일링
        total_loss = loss + (lambda_corr * loss_corr)

        # 4. 단일 Backward
        # 이 한 번의 호출로 Corrector -> Main Model 전체를 관통하며 그래디언트가 흐름
        total_loss.backward()
        ...
        return y_pred, cr_pred, states, h_seqs, smoothing_ratios, loss, sub_losses, loss_corr
    ...
    def infer_batch(self, character, batch_size, eod_stop = False, eod_slice=False, bias = 0, tf_sampling=False, return_preds=False, use_corrector=True, null_correction=False):
        ...
        while i_step < self.max_steps:
            ...
            #     next_var_x, next_var_y, hov, eod = revised_gmm_batch_pt(*parse_pred(y_pred, self.hp))
            # else:
            ...
            # [수정된 부분] 
            # 불필요한 분기문(if tf_sampling)과 Numpy 변환 코드를 모두 날려버리고,
            # 우리가 잘 만들어둔 PyTorch 텐서용 함수 하나로 통일
            parsed_y = parse_pred(y_pred, self.hp) # 튜플 반환
            
            # Gumbel-Softmax가 적용된 PyTorch 함수 사용 (GPU 메모리 내에서 즉시 계산됨)
            # numpy_output=False로 주어 텐서 상태를 유지합니다.
            next_var_x, next_var_y, hov, eod = sample_gmm_batch_pt(*parsed_y, numpy_output=False)
            ...
            # (이제 torch.as_tensor 변환 코드들은 필요 없어졌지만, 혹시 모르니 타입/디바이스를 맞추는 용도로 놔둬도 무방...)
            next_var_x = next_var_x.to(dtype=torch.float32, device=device)
            next_var_y = next_var_y.to(dtype=torch.float32, device=device)
            hov = hov.to(dtype=torch.float32, device=device)
            eod = eod.to(dtype=torch.float32, device=device)
...
class Corrector(nn.Module):
    def __init__(self, hparams):
        ...
        # AdamW
        # [삭제 1] Corrector 전용 Optimizer 제거! 
        # 통합 학습에서는 Main Model(Model 클래스)의 Optimizer가 Corrector 가중치까지 한 번에 업데이트
    ...
    def one_step(self, inputs, states, parse_outputs=True):
        ...
        if parse_outputs:
            ...
            if self.hp['use_corrector_output_gmm']:
                ...
                var_x_corrections, var_y_corrections = revised_gmm_batch_pt(*parsed_preds, only_gmm=True)
