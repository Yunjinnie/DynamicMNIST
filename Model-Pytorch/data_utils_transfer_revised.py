...
class DataLoader():
    def __init__(self, hparams):
        ...
        self.path_digit = self.hp['path_velocity_dataset_digit']
        self.path_char = self.hp['path_velocity_dataset_char']
        self.path_home_dataset = self.hp['path_home_dataset']
        #self.character_set = list(self.hp['character_set'])
        # 순서가 매번 sorted()에 의해 유동적으로 바뀌는 것을 막기 위해 명시적 선언
        digits = [str(i) for i in range(10)]
        alphabets = list("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")

        # 전역 캐릭터 셋을 항상 고정된 순서(숫자 10개 + 알파벳 52개 = 총 62개)로 정렬
        self.character_set = digits + alphabets
        ...
        # 2. 숫자와 알파벳을 결합하여 '단일 정규화 스케일'을 추출
        ...
        # 3. 딕셔너리 형태로 digit과 alphabet 데이터를 메모리에 각각 따로 올려두기
        #self.datasets = self.load_datasets()
        self.datasets = {
            'digit': self.load_datasets(self.path_digit),
            'char': self.load_datasets(self.path_char)
        }
    ...
    def compute_norm_stat(self):
        # 핵심: 숫자와 알파벳 CSV를 모두 읽어서 합친 뒤 정규화 기준을 만들어야 함
        df_digit = pd.read_csv(self.path_digit)
        df_alpha = pd.read_csv(self.path_char)
        #df_csv = pd.read_csv(self.path_velocity_dataset)
        df_csv = pd.concat([df_digit, df_alpha], ignore_index=True)
        ...
        for i, row in df_csv.iterrows():
            ...
            # t_arr = np.asarray(df.iloc[:, 1]) # 2번째 컬럼이 time_ms라고 가정
            # dt = np.diff(t_arr, prepend=t_arr[0])
            # dt = np.where(dt == 0, 1.0, dt) # 0으로 나눠지는 것 방지 (최소 1ms)

            # # x, y로부터 dx, dy 계산
            # if 'dx' not in df.columns:
            #     x_arr = np.asarray(df.x)
            #     y_arr = np.asarray(df.y)
            #     #dx_arr = np.diff(x_arr, prepend=x_arr[0]) # 길이를 맞추기 위해 prepend
            #     #dy_arr = np.diff(y_arr, prepend=y_arr[0])
            #     dx_raw = np.diff(x_arr, prepend=x_arr[0]) / dt  # dt로 나누어 속도(단위 시간당 변화량)화
            #     dy_raw = np.diff(y_arr, prepend=y_arr[0]) / dt
            # else:
            #     dx_arr = np.asarray(df.dx)
            #     dy_arr = np.asarray(df.dy)

            # # d2x, d2y 계산
            # ## 가속도(d2x, d2y) 계산할 때도 dt로 한 번 더 나눠줌
            # if 'd2x' not in df.columns:
            #     #d2x_arr = np.diff(dx_arr, prepend=dx_arr[0])
            #     #d2y_arr = np.diff(dy_arr, prepend=dy_arr[0])
            #     d2x_raw = np.diff(dx_raw, prepend=dx_raw[0]) / dt
            #     d2y_raw = np.diff(dy_raw, prepend=dy_raw[0]) / dt
            # else:
            #     d2x_arr = np.asarray(df.d2x)
            #     d2y_arr = np.asarray(df.d2y)

            # # 첫 번째 포인트 제외하고 추가
            # dx_seqs.append(dx_arr[1:])
            # dy_seqs.append(dy_arr[1:])
            # d2x_seqs.append(d2x_arr[1:])
            # d2y_seqs.append(d2y_arr[1:])

        TARGET_DT = 5.0 # 기준 단위 시간

        for i, row in df_csv.iterrows():
            csv_path = os.path.join(self.path_home_dataset, row.csv_path)
            df = pd.read_csv(csv_path)
            
            # 1. 리샘플링 적용 (균일한 간격의 x, y 도출)
            x_arr, y_arr, _, _ = self.resample_trajectory(df, target_dt=TARGET_DT)
            
            # 2. 고정된 단위 시간(TARGET_DT)으로 나누어 속도/가속도 추출
            dx_raw = np.diff(x_arr, prepend=x_arr[0]) / TARGET_DT
            dy_raw = np.diff(y_arr, prepend=y_arr[0]) / TARGET_DT
            
            d2x_raw = np.diff(dx_raw, prepend=dx_raw[0]) / TARGET_DT
            d2y_raw = np.diff(dy_raw, prepend=dy_raw[0]) / TARGET_DT

            # 첫 번째 포인트 제외하고 추가
            dx_seqs.append(dx_raw[1:])
            dy_seqs.append(dy_raw[1:])
            d2x_seqs.append(d2x_raw[1:])
            d2y_seqs.append(d2y_raw[1:])
    ...
    def resample_trajectory(self, df, target_dt=5.0):
        time_original = np.asarray(df.iloc[:, 1]) # 2번째 컬럼이 time_ms
        x_original = np.asarray(df.x)
        y_original = np.asarray(df.y)
        
        # 데이터가 너무 짧으면 보간이 불가능하므로 원본 반환
        if len(time_original) < 4:
            return x_original, y_original, df.get('hover', None), df.get('pressure', None)
            
        # 새로운 균일 시간 배열 (target_dt 간격)
        time_uniform = np.arange(time_original[0], time_original[-1], target_dt)
        
        # x, y 스플라인 보간 (Cubic)
        fx = interp1d(time_original, x_original, kind='cubic', fill_value="extrapolate")
        fy = interp1d(time_original, y_original, kind='cubic', fill_value="extrapolate")
        x_uniform = fx(time_uniform)
        y_uniform = fy(time_uniform)
        
        # Hover / Pressure 동기화 보간
        h_uniform, p_uniform = None, None
        if 'hover' in df.columns:
            # hover는 0, 1 이진값이므로 nearest(최근접) 보간이 안전합니다.
            fh = interp1d(time_original, df['hover'].values, kind='nearest', fill_value="extrapolate")
            h_uniform = fh(time_uniform)
        elif 'pressure' in df.columns:
            # pressure는 연속값이므로 선형 보간
            fp = interp1d(time_original, df['pressure'].values, kind='linear', fill_value="extrapolate")
            p_uniform = fp(time_uniform)

        return x_uniform, y_uniform, h_uniform, p_uniform

    def load_datasets(self, data_path):
        datasets = dict()

        # 파일이 아예 없거나 경로가 잘못된 경우를 대비한 안전장치
        if not os.path.exists(data_path):
            print(f"[Notice] Path not found: {data_path}. Returning empty dataset.")
            for split in self.splits:
                datasets[split] = (np.array([]), np.array([]), np.array([]), np.array([]))
            return datasets
        
        for split in self.splits:
            df = pd.read_csv(data_path) # self.path_velocity_dataset
            df.character = df.character.astype('string') # or str # 문자열 반환
            df = df[df.split == split]
            
            stack_df_char = list()
            for character in sorted(list(self.hp['character_set'])):
                df_char = df[df.character == character].reset_index(drop = True)

                # 추가 로직: 통합 character_set(62개)을 순회할 때, digit CSV에는 알파벳이 없고 반대도 마찬가지...
                # 해당 문자가 없으면 에러가 나지 않도록 건너뛸 것
                if len(df_char) == 0:
                    continue
            ...
            # [중요 수정 부분] 데이터가 하나도 없으면 concat을 건너뛰고 빈 결과 저장
            if len(stack_df_char) == 0:
                print(f"ℹ[Stage Info] No data found for split '{split}' in {os.path.basename(data_path)}. Skipping...")
                datasets[split] = (np.array([]), np.array([]), np.array([]), np.array([]))
                continue
            ...
            dataset_name = "DIGIT" if "digit" in data_path else "CHAR"
            print(f'Generating [{dataset_name}] Cartesian velocity dataset for {str_verb[split]} ...')
    ...
    def load_dataset(self, split, dataset_type): # = 'digit'
        # dataset_type을 받아서 해당하는 데이터를 꺼내주도록 변경
        return self.datasets[dataset_type][split]
    ...
    def generate_dataset_cartesian(self, df):
        ...
        for i_sample, csv_path in tqdm(enumerate(csv_paths), total=len(csv_paths)):
            ...
            # --- [핵심 수정: 데이터를 읽자마자 리샘플링 적용] ---
            x_arr, y_arr, h_arr, p_arr = self.resample_trajectory(df, target_dt=TARGET_DT)
            
            # 길이가 보간 이후로 바뀌었으므로 sample_len 재설정
            sample_len = len(x_arr) - 1 
            
            # Normalize Position
            x_seq, y_seq = self.normalize_position(x_arr, y_arr)
            x_seq = x_seq[1:]; y_seq = y_seq[1:]

            # 이제 dx, dy가 CSV에 있든 없든 무조건 재계산 (물리적 정합성 보장)
            dx_raw = np.diff(x_arr, prepend=x_arr[0]) / TARGET_DT
            dy_raw = np.diff(y_arr, prepend=y_arr[0]) / TARGET_DT
            ...
            # Norm 스케일링 적용
            dx_seq = dx_raw[1:] / self.dx_norm
            dy_seq = dy_raw[1:] / self.dy_norm
            d2x_seq = d2x_raw[1:] / self.d2x_norm
            d2y_seq = d2y_raw[1:] / self.d2y_norm

            # Hover 및 Pressure 처리 (보간된 결과물 사용)
            if h_arr is not None:
                h_seq = h_arr[1:]
            elif p_arr is not None:
                # 압력이 0보다 크면 그리는 중(0), 0이면 펜이 떠있음(1)
                h_seq = (p_arr[1:] <= 0).astype(np.float32)
            else:
                h_seq = np.zeros(sample_len)
            
            eod_seq = np.zeros(sample_len)
            eod_seq[-1] = 1 
            
            # --- [이하 input_seq, target_seq 조립 과정은 기존 코드와 동일] ---
        ...
        #         continue
        #     # 데이터 로더 내부 수정 제안
        #     # if "digit" in csv_path:
        #     #     self.character_set = [str(i) for i in range(10)] # 숫자만
        #     # else:
        #     #     self.character_set = list("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ") # 알파벳만
        #     character_vector = get_character_vector(character, self.character_set)
        ...
        #     # 1. 위치 및 변위 정보 추출 & 정규화
        #     # dx, dy, d2x, d2y 동적 계산
        #     if 'x' in df.columns and 'dx' not in df.columns:
        #         # i_stroke,time_ms,x,y,xTilt,yTilt,pressure
        #         # x, y만 있는 경우 dx, dy 계산
        #         #dx_arr = np.concatenate([[0], np.diff(x_raw)])
        #         #dy_arr = np.concatenate([[0], np.diff(y_raw)])
        #         #x_seq, y_seq = self.normalize_position(x_raw, y_raw)
        #         dx_raw = np.diff(x_arr, prepend=x_arr[0])
        #         dy_raw = np.diff(y_arr, prepend=y_arr[0])

        #     elif 'dx' in df.columns:
        #         # time_ms,dx,dy,direction,speed,hover
        #         dx_raw = np.asarray(df.dx)
        #         dy_raw = np.asarray(df.dy)
        #         # x, y 없다면 역으로 적분 (cumsum)해서 생성
        #         if 'x' in df.columns:
        #             x_seq, y_seq = self.normalize_position(x_arr, y_arr)
        #         else:
        #             x_cum = np.cumsum(dx_raw)
        #             y_cum = np.cumsum(dy_raw)
        #             x_seq, y_seq = self.normalize_position(x_cum, y_cum)

        #     # 3. 가속도 (d2x, d2y) 계산
        #     # d2x_seq = np.concatenate([[0], np.diff(dx_arr)]) / self.d2x_norm
        #     # d2y_seq = np.concatenate([[0], np.diff(dy_arr)]) / self.d2y_norm

        #     if 'd2x' not in df.columns:
        #         d2x_raw = np.diff(dx_raw, prepend=dx_raw[0])
        #         d2y_raw = np.diff(dy_raw, prepend=dy_raw[0])
        #     else:
        #         d2x_raw = np.asarray(df.d2x)
        #         d2y_raw = np.asarray(df.d2y)
        ...
        #     # 2. Hover 정보 추출 (Pressure vs Hover: Pressure into Hover transform)
        #     if 'hover' in df.columns:
        #         h_seq = np.asarray(df.hover)[1:]
        #     elif 'pressure' in df.columns:
        #         # 압력이 0보다 크면 그리는 중(hover=0), 0이면 펜이 떠있는 것(hover=1)
        #         h_seq = (np.asarray(df.pressure)[1:] <= 0).astype(np.float32)
        #     else:
        #         h_seq = np.zeros(sample_len) # 정보 없으면 일단 0으로 채움            
            
        #     eod_seq = np.zeros(sample_len)
        #     eod_seq[-1] = 1 

            # 이후 로직(target_seq, input_seq 구성)은 기존과 동일
    ...
    def generate_dataset_for_corrector(self, preds_controller, states_controller, h_seqs_controller, inputs_controller, targets_controller, mask_controller):
        ...
        # preds_controller의 마지막 차원 크기를 확인
        # 보통 GMM 파라미터는 60~120차원, 이미 샘플링된 궤적은 5~6차원
        if preds_controller.shape[-1] > 10: 
            # 1. GMM 파라미터가 들어온 경우 (Stage 1, 3) -> 샘플링 필요
            dx, dy, hov, eod = sample_gmm_seqs_pt(preds_controller, self.hp)
        else:
            # 2. 이미 샘플링된 궤적(simulated_pred 등)이 들어온 경우 (Stage 2) -> 그대로 사용
            # preds_controller가 이미 [batch, seq, 5] 형태라고 가정
            dx = preds_controller[:, :, 0]
            dy = preds_controller[:, :, 1]
            hov = preds_controller[:, :, 2]
            eod = preds_controller[:, :, 3]
