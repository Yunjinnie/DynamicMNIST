...
from data_utils import get_character_from_vector, rasterize_dev
...
class Logger:
    ...
    def __init__(self, project_name, run_id, hparams, off_wandb):
        ...
        # Initialize an WandB logging session.
        wandb.init(
        ...
        # Tensorflow log filtering:
        ## '1': INFO messages are not printed.
        ## '2': WARNING messages are not printed.
    ...
    def compute_stat_between_human_sets(self, train_set, val_set, test_set):
        #stat_counter_data_character, stat_counter_data_all = get_stat_data_from_test_set(self.character_set, train_set, self.dx_norm, self.dy_norm)
        ...
        pvalues = {metric:{character:list() for character in self.character_set} for metric in metrics} # self.pvalues = {metric:list() for metric in metrics}
        ...
        for character in self.character_set:
            for metric in metrics:
                ...
                p_range = p_stats.minmax
                q_range = q_stats.minmax
                ...
                pvalues[metric][character].append(pvalue) # self.pvalues[metric].append(pvalue)
    ...
    def make_log_dir(self):
        dir_log = self.hp['dir_log']
        prj_name = self.prj_name
        run_id = self.run_id

        # Create directories required for logging.
        dir_log_path = os.path.join(dir_log, prj_name, run_id)
    ...
    def save_model_now(self):
        return self.behavioral_plausibility()

    def behavioral_plausibility(self):
        '''
        for m in self.core_metrics:
            if np.mean(self.pvalues[m]) < 0.5:
            #if self.pvalue_all[m] < 0.5:
                return False
        return True
        '''
    ...
    def save_model(self, model, epoch_float, iterations):
        model.epochs.assign(int(epoch_float))
        model.iterations.assign(iterations)
        filename = '{}-{}-epoch-{:0{}d}-it-{:0{}d}.tf'.format(
        ...
        if self.accuracy_total_max <= self.accuracy_total:
    ...
    def log_train(self, model, itr, epoch_float, batch, preds, smoothing_ratios, loss, sub_losses, corrector_outputs):
        ...
        # Make smoothing_ratios a single value.
        if tf.size(smoothing_ratios) > 1:
            smoothing_ratios = tf.squeeze(smoothing_ratios, axis=-1)
            mean_smooth_ratio = tf.reduce_sum([tf.reduce_sum(smoothing_ratios[i,:seq_lengths[i]]) for i in tf.range(tf.shape(seq_lengths)[0])]) / tf.cast(tf.reduce_sum(seq_lengths), dtype=smoothing_ratios.dtype)
        else:
        ...
        wandb.log({
            'iterations':itr,
            'epoch':epoch_float,
            'learning_rate':model.lr_schedule(model.optimizer.iterations).numpy(),
            'train/loss/total':tf.get_static_value(loss),
            'train/loss/gmm':tf.get_static_value(loss_gmm),
            'train/loss/hov':tf.get_static_value(loss_hov),
            'train/loss/eod':tf.get_static_value(loss_eod),
            'train/loss/corrector':tf.get_static_value(cr_loss),
            ...
            'train/smoothing_ratio/mean':mean_smooth_ratio,
        }, step=itr)

        self.samples_train_synth = dict()
        batch_size = inputs.shape[0]
        for i in range(batch_size):
            character = get_character_from_vector(inputs[i,0,-self.v_character_dim:], self.character_set)
            if character not in self.samples_train_synth.keys():
                self.samples_train_synth[character] = {
                    'target_sample':targets[i],
                    'pred_sample':preds[i],
                    'seq_length_sample':seq_lengths[i]
                }
    ...
    def log_train_synth(self, itr, epoch_float, batch, preds, loss, sub_losses):
        ...
        for character, sample in self.samples_train_synth.items():
            # Select a random example. Use the last mini-batch.
            ...
            # Log the target GIF image to WandB.
            ...
            # Construct dots from a sequence of velocities.
            ...
            # Log the predicted GIF image by the teacher-forcing mode to WandB.
            wandb.log({'train/image-gif-prediction-TF/image/prediction/character-{}'.format(character): wandb.Image(gif_path)}, step=itr)

            # Get the mean absolute errors of direction and speed between targets and sampling means.
            ...
            # Get the mean absolute errors of direction and speed between targets and sampled predictions
    ...
    def log_val_eval(self, batch, preds, smoothing_ratios, loss, sub_losses, corrector_outputs):
        ...
        # Accumulate metrics.
        self.sum_loss_val += tf.get_static_value(loss)
        self.sum_loss_gmm_val += tf.get_static_value(loss_gmm)
        self.sum_loss_hov_val += tf.get_static_value(loss_hov)
        self.sum_loss_eod_val += tf.get_static_value(loss_eod)
        self.sum_loss_cr += tf.get_static_value(cr_loss)
        ...
        self.sum_mean_smooth_ratio += mean_smooth_ratio
    ...
    def log_val_synth(self, itr, epoch_float, batch, preds, loss, sub_losses, model):
        ...
        for character, sample in self.samples_val_synth.items():
            # Randomly select a sample from the last mini-batch of the validation set.
            target_sample = sample['target_sample']
            pred_sample = sample['pred_sample']
            seq_length_sample = sample['seq_length_sample']
            ...
            # Construct dots from a sequence of direction and speed
            df_dots = get_dot_seq_from_dx_dy(df_v, self.w_img, self.h_img, self.dt)

            # Generate an image from dots, hovering and EOD(End Of Drawing) indicators.
            ...
            # Get the mean absolute errors of dx and dy between targets and sampling means.
            dx_mae_target_gmm_mean, dy_mae_target_gmm_mean = get_target_gmm_mean_distance(target_sample, pred_sample, seq_length_sample, self.dx_mean, self.dx_norm, self.dy_mean, self.dy_norm, self.hp)
            dx_std_mean, dy_std_mean, corr_mean, corr_abs_mean, det_mean = get_gmm_mean_std(pred_sample, seq_length_sample, self.dx_norm, self.dy_norm, self.hp)

            # Get the mean absolute errors of dx and dy.
            dx_seq_trgt = df_v_trgt.dx.to_numpy()
            dy_seq_trgt = df_v_trgt.dy.to_numpy()
            dx_seq_pred = df_v.dx.to_numpy()
            dy_seq_pred = df_v.dy.to_numpy()
    ...
    def log_test_synth(self, itr, epoch_float, model, test_set):
        ...
        stat_test_data_character, stat_test_data_all = get_stat_data_from_test_set(self.character_set, test_set, self.dx_norm, self.dy_norm)
        #metrics = ['seq_lengths', 'dspeed_means', 'areas']
        metrics = list(stat_test_data_all.keys())
        #metrics = ['speed_means', 'durations', 'lengths', 'diameters']
        ...
        for character in self.character_set:
            ...
            dx_std_mean, dy_std_mean, corr_mean, corr_abs_mean, det_mean = get_gmm_mean_std(preds, seq_lengths, model.dx_norm, model.dy_norm, model.hp)
        ...
        wandb.log({
            'test/variability/dx_std_mean/total'.format(character): np.mean(dx_std_means),
            'test/variability/dy_std_mean/total'.format(character): np.mean(dy_std_means),
            'test/variability/corr_means/total'.format(character): np.mean(corr_means),
            'test/variability/corr_abs_means/total'.format(character): np.mean(corr_abs_means),
            'test/variability/det_means/total'.format(character): np.mean(det_means),
        }, step=itr)
        ...
        wandb.log({
            'test/behavioral_plausibility':int(self.behavioral_plausibility()),
        ...
        wandb.log({
            'test/dtw-classification/prediction-SF/accuracy':self.accuracy_total_dtw,
        }, step=itr)
        for character, accuracy in accuracy_character.items():
            wandb.log({
                'test/dtw-classification/prediction-SF/accuracy-character-{}'.format(character):accuracy,
            }, step=itr)

        if self.accuracy_total_dtw_test is None:
            self.accuracy_total_dtw_test, self.accuracy_character = self.dtw_classifier.classify(targets_test, characters_test)
        print(f"[Test Set Target] DTW Total Accuracy: {self.accuracy_total_dtw_test:.4f}")
        wandb.log({
            'test/dtw-classification/test_set/accuracy':self.accuracy_total_dtw_test,
        }, step=itr)

        for character, accuracy in self.accuracy_character.items():
            wandb.log({
                'test/dtw-classification/test_set/accuracy-character-{}'.format(character):accuracy,
            }, step=itr)
        ...
        wandb.log({
            'test/dtw-classification/prediction-SF/accuracy_gap/total':accuracy_gap,
        }, step=itr)
        for character in accuracy_character.keys():
            accuracy_gap = self.accuracy_character[character] - accuracy_character[character]
            wandb.log({
                'test/dtw-classification/prediction-SF/accuracy_gap/character-{}'.format(character):accuracy_gap,
            }, step=itr)
        ...
        wandb.log({
            'test/cnn-classification/prediction-SF/accuracy':self.accuracy_total_cnn,
        }, step=itr)
        for character, precision in precisions.items():
            wandb.log({
                'test/cnn-classification/prediction-SF/precision-character-{}'.format(character):precision,
            }, step=itr)
        for character, recall in recalls.items():
            wandb.log({
                'test/cnn-classification/prediction-SF/recall-character-{}'.format(character):recall,
            }, step=itr)
        for character, f1score in f1scores.items():
            wandb.log({
                'test/cnn-classification/prediction-SF/f1score-character-{}'.format(character):f1score,
            }, step=itr)

        if self.accuracy_total_cnn_test is None:
            self.accuracy_total_cnn_test, self.precisions_cnn_test, self.recalls_cnn_test, self.f1scores_cnn_test = self.cnn_classifier.classify(targets_test, characters_test)
        ...
        for character, precision_cnn_test in self.precisions_cnn_test.items():
            wandb.log({
                'test/cnn-classification/test_set/precision-character-{}'.format(character):precision_cnn_test,
            }, step=itr)
            wandb.log({
                'test/ccn-classification/prediction-SF/gap/precision-character-{}'.format(character):(precision_cnn_test - precisions[character]),
            }, step=itr)

        for character, recall_cnn_test in self.recalls_cnn_test.items():
            wandb.log({
                'test/cnn-classification/test_set/recall-character-{}'.format(character):recall_cnn_test,
            }, step=itr)
            wandb.log({
                'test/ccn-classification/prediction-SF/gap/recall-character-{}'.format(character):(recall_cnn_test - recalls[character]),
            }, step=itr)

        for character, f1score_cnn_test in self.f1scores_cnn_test.items():
            wandb.log({
                'test/cnn-classification/test_set/f1score-character-{}'.format(character):f1score_cnn_test,
            }, step=itr)
            wandb.log({
                'test/ccn-classification/prediction-SF/gap/f1score-character-{}'.format(character):(f1score_cnn_test - f1scores[character]),
            }, step=itr)
