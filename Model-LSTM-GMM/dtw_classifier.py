...
from data_utils import rasterize_dev
...
class DTWClassifier:
    def __init__(self, hp, avg_set, log_dir, dx_scale, dy_scale):
        ...
        self.inputs_avg = self.avg_set[0]
        self.targets_avg = self.avg_set[1]
        self.seq_lengths_avg = self.avg_set[2]
        self.characters_avg = self.avg_set[3]
        self.log_dir = os.path.join(log_dir, 'dtw_classifier')
        os.makedirs(self.log_dir, exist_ok=True)
        self.dx_scale = dx_scale
        self.dy_scale = dy_scale
        self.character_set = list(hp['character_set'])
    ...
    def cluster_motor_programs(self):
        ...
        for character, n_motor_programs in self.motor_programs.items():
            ...
            if n_motor_programs == 1:
                indexes_by_motor_programs[character][0] += list(indexes_character)
                continue

            targets_character = self.targets_avg[indexes_character]
            n_samples = targets_character.shape[0]
            ...
            #seqs, groups = shuffle(seqs, groups, random_state=self.seed)
            ...
            for i_cluster in np.unique(clusters_pred):
                ...
                indexes_by_motor_programs[character][i_cluster] += list(indexes_character[indexes_cluster])
    ...
    def compute_barycenters(self):
        ...
        # barycenters[character][motor_program] = barycenter
            ...
            for motor_program, indexes_mp in enumerate(indexes_mps):
                ...
                for idim in range(self.n_dims):
                    ...
                    for i, seq in enumerate(seqs):
                        ...
                        '''
                        if character == 4:
                            plt.yticks(fontsize=18, alpha=1.0)
                        else:
                            plt.yticks(fontsize=18, alpha=0.0)
                        #'''
                        plt.xticks(fontsize=18)
                        if idim == self.n_dims - 1:
                            # xticks only for the bottom plot
                    ...
                    #plt.plot(dtw_barycenter[:,idim]*dim_scales[idim], "r-")
    ...
    def classify(self, seqs_eval, labels_eval, get_eval_pairs=False):
        ...
        for character in self.character_set:
            ...
            for i_sample in range(n_samples):
                ...
                x = accumulate_sequence(dx)
                y = accumulate_sequence(dy)
                x_seq_sample = np.stack([dx,dy,x,y], axis=-1)
        ...
        accuracy_total = n_correct_total / n_samples_total
...
def compute_barycenter_outputs(log_dir, character, y_pred, seq_lengths, dx_scale, dy_scale, hparams):
    ...
    dx_mean_weighted_sum = tf.reduce_sum(tf.multiply(pi, dx_mean), axis=-1)
    dy_mean_weighted_sum = tf.reduce_sum(tf.multiply(pi, dy_mean), axis=-1)
    ...
    for i_sample in range(n_samples):
        seq_length = seq_lengths[i_sample]

        dx_mean = dx_mean_weighted_sum[i_sample,:seq_length].numpy()
        dy_mean = dy_mean_weighted_sum[i_sample,:seq_length].numpy()
        dx_std = dx_std_weighted_sum[i_sample,:seq_length].numpy()
        dy_std = dy_std_weighted_sum[i_sample,:seq_length].numpy()
        corr = corr_weighted_sum[i_sample,:seq_length].numpy()

        seq = np.stack([dx_mean,dy_mean,dx_std,dy_std,corr], axis=-1)
        seqs.append(seq)
